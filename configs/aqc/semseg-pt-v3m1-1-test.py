...
num_worker = 8# 数据加载时使用的工作线程数量
#enable_amp = True # 是否启用自动混合精度
enable_amp = False

ignore_index = 0 # 忽略的标签索引，通常用于忽略背景或无效标签
...
model = dict(
    ...
    criteria=[
        dict(type="CrossEntropyLoss", loss_weight=1.0),  # 交叉熵损失及其权重
        dict(type="LovaszLoss", mode="multiclass", loss_weight=1.0),  # Lovasz 损失及其权重
    ],
)
